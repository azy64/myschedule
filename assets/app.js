...
const transformDataToPatientByDate = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    const item = { numberOfPatient: 1, arrivalDate: data[i].arrivalDate };
    for (let j = 0; j < data.length && j != i; j++) {
      if (item.arrivalDate === data[j].arrivalDate) {
        item["numberOfPatient"] += 1;
      }
    }
    //console.log("item before added",item);
    //const tab = newDataPatient.filter((el)=>el.arrivalDate===item.arrivalDate)
    //if(tab.length===0){
    let value = false;
    for (let k = 0; k < newDataPatient.length; k++) {
      if (newDataPatient[k].arrivalDate === item.arrivalDate) {
        ...
        //console.log("valeur detecté:", item," et ",newDataPatient[k])
        break;
      }
    }
    if (value === false)
